...
cl_application::cl_application(cl_base* p_head):cl_base(p_head) { // конструктор базового класса
}
...
void cl_application::build_tree_objects() { // построение иерархии дерева
  ...
  set_name(s_head_name); // // присваеваем его текущему объекту
  ...
  while (true) { // следом идет ввод бесконечных пар объектов
    ...
    if (p_head -> set_sub_object(s_sub_name)) == nullptr && s_head_name == (p_head -> get_name())
